...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    const curItem = res[i];
    let prevIndex = i - 1;
    while (res[prevIndex] > curItem && prevIndex >= 0) {
      res[prevIndex + 1] = res[prevIndex];
      prevIndex -= 1;
    }
    res[prevIndex + 1] = curItem;
  }
  return res;
}
